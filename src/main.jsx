...
function PWAInstallPrompt() {
  ...
  const handleInstall = async () => {
    ...
    if (outcome === 'accepted') console.log('Użytkownik zainstalował aplikację')
    ...
    <div className="fixed bottom-20 left-4 right-4 z-50 bg-white rounded-lg shadow-lg p-4 border border-gray-200">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={dismiss} className="px-3 py-1.5 text-xs text-gray-400 hover:text-gray-600 transition-colors">
            Nie teraz
          </button>
          <button onClick={handleInstall} className="px-4 py-2 bg-violet-600 text-white rounded-lg font-medium hover:bg-violet-700 transition-colors">
            Instaluj
          </button>
        </div>
      </div>
