...
import {
  ...
  Edit2, Archive, RotateCcw, Image as ImageIcon, Calendar, TrendingUp, Save, FlaskConical,
} from 'lucide-react';
import { auth, db, SUPER_ROOT, MYDEVIL_URL, COL_RECIPES, COL_CATEGORIES } from '../firebase';
...
const CALC_OPTIONS = [
  { value: 'nalewki',    label: 'Nalewkarz Master' },
...
const StatCard = ({ icon: Icon, label, value, accent }) => (
  <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-6 flex flex-col gap-3">
    <div className="flex items-center gap-2">
      <Icon size={16} className={accent ? 'text-violet-500' : 'text-[var(--text-dim)]'} />
...
const AdminPanel = ({ allUsers, categories, ads, allRecipes = [], updatePlayerPlan, toggleAdmin, deleteUserAccount, onAddRecipe }) => {
  ...
  const stats = useMemo(() => ({
    ...
    newUsersWeek: allUsers.filter(u => { const ts = parseTs(u.createdAt); return ts !== null && ts >= sevenDaysAgo; }).length,
  ...
  const recentUsers = useMemo(() =>
    [...allUsers].filter(u => u.createdAt).sort((a, b) => (parseTs(b.createdAt) ?? 0) - (parseTs(a.createdAt) ?? 0)).slice(0, 10),
    [allUsers]
  );

  const pendingRecipes  = useMemo(() => allRecipes.filter(r => r.ownerId !== 'ADMIN' && !r.blocked), [allRecipes]);
  const activeAdsDash   = useMemo(() => ads.filter(ad => ad.active), [ads]);
  const activeAdsList   = useMemo(() => ads.filter(ad => { if (ad.archived) return false; const end = ad.endDate?.seconds ? ad.endDate.seconds * 1000 : null; return !end || end >= now; }), [ads, now]);
  const archiveAdsList  = useMemo(() => ads.filter(ad => { if (ad.archived) return true; const end = ad.endDate?.seconds ? ad.endDate.seconds * 1000 : null; return end && end < now; }), [ads, now]);

  const toggleAdCalc = (val) => setAdForm(f => ({
    ...f, calculators: (f.calculators || []).includes(val) ? f.calculators.filter(c => c !== val) : [...(f.calculators || []), val],
  }));

  const openNewAdForm  = () => { setAdForm(EMPTY_AD); setEditAdId(null); setShowAdForm(true); };
  const openEditAdForm = (ad) => {
    setAdForm({ title: ad.title || ad.content || '', imageUrl: ad.imageUrl || '', targetUrl: ad.targetUrl || '',
      startDate: tsToInputDate(ad.startDate), endDate: tsToInputDate(ad.endDate), active: ad.active ?? true, calculators: ad.calculators || [] });
    setEditAdId(ad.id); setShowAdForm(true);
  };
  ...
  const handleSaveAd = async () => {
    ...
    const payload = { title: adForm.title.trim(), imageUrl: adForm.imageUrl, targetUrl: adForm.targetUrl,
      startDate: inputDateToDate(adForm.startDate), endDate: inputDateToDate(adForm.endDate),
      active: adForm.active, archived: false, calculators: adForm.calculators || [] };
    if (editAdId) await updateDoc(doc(db, 'ads', editAdId), payload);
    else await addDoc(collection(db, 'ads'), { ...payload, createdAt: serverTimestamp() });
    closeAdForm();
  };
  ...
  const handleUploadImage = async (e) => {
    ...
    try { const res = await fetch(MYDEVIL_URL, { method: 'POST', body: fd }); const data = await res.json(); if (data.url) setAdForm(f => ({ ...f, imageUrl: data.url })); }
    catch { alert('Błąd uploadu.'); } finally { setUploadingImg(false); }
  };

  const loadUserRecipes = async (userId) => {
    if (userRecipesMap[userId]) return;
    const snap = await getDocs(query(collection(db, COL_RECIPES), where('ownerId', '==', userId)));
  ...
  const handleSaveCalculatorPlan = async (userId, calc, plan) =>
    await updateDoc(doc(db, 'users', userId), { [`calculatorPlans.${calc}.plan`]: plan });

  const handleBlockRecipe = async (recipeId, block) => {
    await updateDoc(doc(db, COL_RECIPES, recipeId), { blocked: block });
    setUserRecipesMap(prev => {
      const updated = { ...prev };
      for (const uid in updated) updated[uid] = updated[uid].map(r => r.id === recipeId ? { ...r, blocked: block } : r);
      return updated;
    });
  ...
  const exportUsersCSV = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'uzytkownicy.csv'; a.click();
    ...
    <main className="max-w-7xl mx-auto p-4 md:p-10 animate-in fade-in slide-in-from-bottom-4 duration-500 text-left">
        ...
        <div className="flex bg-[var(--bg)] border border-[var(--border)] p-1.5 rounded-3xl w-fit">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setAdminSubTab(t.id)}
              className={`px-8 py-3 rounded-2xl text-xs font-black uppercase transition-all ${adminSubTab === t.id ? 'bg-[var(--bg-card)] shadow text-violet-500' : 'text-[var(--text-dim)]'}`}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      {adminSubTab === 'dashboard' && (
        <div className="space-y-8">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            ...
            <StatCard icon={UserPlus}   label="Nowi (7 dni)"           value={stats.newUsersWeek} />
            ...
            <div className="divide-y divide-[var(--border)]">
              ...
              {recentUsers.map(u => (
                <div key={u.id} className="flex items-center justify-between py-3">
                  ...
                  <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-full shrink-0 ${u.plan && u.plan !== 'free' ? 'bg-violet-900/30 text-violet-400' : 'bg-[var(--bg-input)] text-[var(--text-dim)]'}`}>
                    {u.plan || 'free'}
                  </span>
                </div>
              ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-6">
              <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-dim)] mb-4">Aktywne reklamy ({activeAdsDash.length})</p>
              {activeAdsDash.length === 0 ? (
                <p className="text-xs text-[var(--text-dim)] py-4 text-center font-bold uppercase">Brak aktywnych reklam</p>
              ) : (
                <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                  {activeAdsDash.map(ad => (
                    <div key={ad.id} className="flex items-center gap-3 p-3 bg-[var(--bg)] border border-[var(--border)] rounded-2xl">
                      ...
                      <button onClick={() => updateDoc(doc(db, 'ads', ad.id), { active: false })}
                        className="flex items-center gap-1 px-3 py-1.5 bg-[var(--bg-input)] text-[var(--text-dim)] rounded-xl text-[9px] font-black uppercase hover:opacity-70 shrink-0">
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-6">
            ...
            <div className="flex flex-wrap gap-3">
              <button onClick={() => onAddRecipe?.()}
                className="flex items-center gap-2 px-5 py-3 bg-violet-600 text-white rounded-2xl text-xs font-black uppercase hover:bg-violet-700 transition-all">
                <Plus size={14} /> Dodaj recepturę wzorcową
              </button>
              <button onClick={() => { setAdminSubTab('ads'); openNewAdForm(); }}
                className="flex items-center gap-2 px-5 py-3 bg-[var(--bg-input)] text-[var(--text-dim)] rounded-2xl text-xs font-black uppercase hover:opacity-70 transition-all">
      ...
      {adminSubTab === 'users' && (
        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[3rem] p-6 md:p-10 shadow-xl">
          <div className="divide-y divide-[var(--border)]">
            {allUsers.map(u => {
              const calcState = userCalcState[u.id] || { calc: 'nalewki', plan: 'free' };
                ...
                <React.Fragment key={u.id}>
                  <div className="flex items-center justify-between py-5 cursor-pointer hover:bg-[var(--bg)] px-2 -mx-2 rounded-2xl transition-colors"
                    ...
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      {u.isAdmin && <span className="text-[8px] font-black bg-violet-900/30 text-violet-400 px-2 py-0.5 rounded-full uppercase shrink-0">Admin</span>}
                    </div>
                      ...
                      {u.email !== SUPER_ROOT ? (
                        <>
                          <button onClick={() => toggleAdmin(u.id, u.isAdmin)}
                            className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase ${u.isAdmin ? 'bg-violet-900/30 text-violet-400' : 'bg-[var(--bg-input)] text-[var(--text-dim)]'}`}>
                        ...
                        <span className="text-[10px] font-black text-violet-500 uppercase px-3">Właściciel</span>
                      )}
                    </div>
                  </div>
                  ...
                  {expandedUserId === u.id && (
                    <>
                      <div className="bg-[var(--bg)] border border-[var(--border)] rounded-2xl p-4 mt-1 mb-2 mx-1">
                            ...
                            {CALC_OPTIONS.map(c => {
                              ...
                              return (
                                <div key={c.value} className="flex items-center justify-between text-[10px] px-3 py-2 bg-[var(--bg-card)] border border-[var(--border)] rounded-xl">
                                  ...
                                  <span className={`font-black uppercase px-2 py-0.5 rounded-full text-[9px] ${cp.plan !== 'free' ? 'bg-violet-900/30 text-violet-400' : 'bg-[var(--bg-input)] text-[var(--text-dim)]'}`}>
                                    {cp.plan}
                                  </span>
                                </div>
                              );
                        ...
                        <div className="flex items-center gap-2">
                          <select value={calcState.calc} onChange={e => setUserCalcState(prev => ({ ...prev, [u.id]: { ...calcState, calc: e.target.value } }))}
                          ...
                          <select value={calcState.plan} onChange={e => setUserCalcState(prev => ({ ...prev, [u.id]: { ...calcState, plan: e.target.value } }))}
                            className="flex-1 bg-[var(--bg-card)] border border-[var(--border)] text-[var(--text)] p-2 rounded-xl text-[10px] font-black uppercase outline-none">
                            {PLAN_OPTIONS.map(p => <option key={p} value={p}>{p}</option>)}
                          </select>
                          <button onClick={() => handleSaveCalculatorPlan(u.id, calcState.calc, calcState.plan)}
                            className="flex items-center gap-1.5 px-3 py-2 bg-violet-600 text-white rounded-xl text-[10px] font-black uppercase hover:bg-violet-700 shrink-0">
                            <Save size={11} /> Zapisz
                          </button>
                        </div>
                      </div>
                              ...
                              <div key={r.id} className={`flex items-center justify-between p-3 rounded-xl border ${r.blocked ? 'bg-red-900/20 border-red-900/30' : 'bg-[var(--bg-card)] border-[var(--border)]'}`}>
                                <div className="min-w-0 flex-1">
                                  ...
                                  <p className="text-[10px] text-[var(--text-dim)] font-bold uppercase tracking-wider mt-0.5">
                                    {r.category}{r.calc_final_strength ? ` · ${Number(r.calc_final_strength).toFixed(1)}%` : ''}
                                  </p>
                                </div>
                                <div className="flex items-center gap-2 ml-3 shrink-0">
                                  <button onClick={() => setPreviewRecipe(r)}
                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--bg-input)] text-[var(--text-dim)] rounded-xl text-[10px] font-black uppercase hover:opacity-70">
                                    <Eye size={11} /> Podgląd
                                  </button>
                                  {r.blocked ? (
                                    <button onClick={() => handleBlockRecipe(r.id, false)}
                                      className="flex items-center gap-1.5 px-3 py-1.5 bg-green-900/30 text-green-400 rounded-xl text-[10px] font-black uppercase hover:bg-green-700 hover:text-white">
                                      <Unlock size={11} /> Odblokuj
                                    </button>
                                  ) : (
                                    <button onClick={() => handleBlockRecipe(r.id, true)}
                                      className="flex items-center gap-1.5 px-3 py-1.5 bg-red-900/20 text-red-400 rounded-xl text-[10px] font-black uppercase hover:bg-red-600 hover:text-white">
      ...
      {adminSubTab === 'ads' && (
        <div className="space-y-6">
          {showAdForm && (
            <div className="bg-[var(--bg-card)] border border-violet-500/40 rounded-[2.5rem] p-8 shadow-xl">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-black uppercase text-[var(--text)]">{editAdId ? 'Edytuj reklamę' : 'Nowa reklama'}</h3>
                <button onClick={closeAdForm} className="p-2 bg-[var(--bg-input)] text-[var(--text-dim)] rounded-xl hover:text-[var(--text)]"><X size={16} /></button>
              </div>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input className={inputCls} placeholder="https://..." value={adForm.imageUrl}
                      onChange={e => setAdForm(f => ({ ...f, imageUrl: e.target.value }))} />
                    <label className="flex items-center gap-1.5 px-4 py-3 rounded-xl text-xs font-black uppercase cursor-pointer shrink-0 bg-[var(--bg-input)] text-[var(--text-dim)] hover:opacity-70">
                      <ImageIcon size={13} />
                  ...
                  {adForm.imageUrl && <img src={adForm.imageUrl} alt="" className="mt-2 h-20 object-cover rounded-xl border border-[var(--border)]" />}
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[var(--text-dim)] mb-1">Link docelowy</label>
                  <input className={inputCls} placeholder="https://..." value={adForm.targetUrl}
                  ...
                  <div className="flex flex-wrap gap-5">
                    {CALC_OPTIONS.map(c => (
                      <label key={c.value} className="flex items-center gap-2 cursor-pointer select-none">
                        <input type="checkbox" checked={(adForm.calculators || []).includes(c.value)} onChange={() => toggleAdCalc(c.value)} className="w-4 h-4 accent-violet-600" />
                ...
                <div className="flex items-center justify-between pt-2">
                  <button onClick={() => setAdForm(f => ({ ...f, active: !f.active }))}
                    className={`flex items-center gap-2 px-5 py-3 rounded-2xl text-xs font-black uppercase border ${adForm.active ? 'bg-green-900/20 text-green-400 border-green-900/30' : 'bg-[var(--bg-input)] text-[var(--text-dim)] border-[var(--border)]'}`}>
                    <div className={`w-3 h-3 rounded-full ${adForm.active ? 'bg-green-400' : 'bg-[var(--text-dim)]'}`} />
                    {adForm.active ? 'Aktywna' : 'Nieaktywna'}
                  </button>
                  <div className="flex gap-2">
                    <button onClick={closeAdForm} className="px-6 py-3 bg-[var(--bg-input)] text-[var(--text-dim)] rounded-2xl text-xs font-black uppercase hover:opacity-70">Anuluj</button>
                    <button onClick={handleSaveAd} className="px-6 py-3 bg-violet-600 text-white rounded-2xl text-xs font-black uppercase hover:bg-violet-700">
          ...
          <div className="flex items-center justify-between">
            <div className="flex bg-[var(--bg)] border border-[var(--border)] p-1 rounded-2xl">
              <button onClick={() => setAdSubTab('active')}
                className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${adSubTab === 'active' ? 'bg-[var(--bg-card)] text-violet-500 shadow' : 'text-[var(--text-dim)]'}`}>
                Aktywne ({activeAdsList.length})
              </button>
              <button onClick={() => setAdSubTab('archive')}
                className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${adSubTab === 'archive' ? 'bg-[var(--bg-card)] text-violet-500 shadow' : 'text-[var(--text-dim)]'}`}>
                Archiwum ({archiveAdsList.length})
              </button>
            </div>
            {!showAdForm && (
              <button onClick={openNewAdForm} className="flex items-center gap-2 px-5 py-3 bg-violet-600 text-white rounded-2xl text-xs font-black uppercase hover:bg-violet-700 shadow-lg">
          ...
          {(adSubTab === 'active' ? activeAdsList : archiveAdsList).length === 0 ? (
            ...
            <div className="space-y-3">
              {(adSubTab === 'active' ? activeAdsList : archiveAdsList).map(ad => {
                ...
                return (
                  <div key={ad.id} className={`bg-[var(--bg-card)] border rounded-3xl p-5 ${ad.active && !isExpired ? 'border-[var(--border)]' : 'border-[var(--border)] opacity-60'}`}>
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          ...
                          {isExpired ? <span className="text-[8px] font-black bg-[var(--bg-input)] text-[var(--text-dim)] px-2 py-0.5 rounded-full uppercase shrink-0">Wygasła</span>
                            : isUpcoming ? <span className="text-[8px] font-black bg-blue-900/30 text-blue-400 px-2 py-0.5 rounded-full uppercase shrink-0">Zaplanowana</span>
                            : ad.active ? <span className="text-[8px] font-black bg-green-900/30 text-green-400 px-2 py-0.5 rounded-full uppercase shrink-0">Aktywna</span>
                        ...
                        {(ad.startDate || ad.endDate) && (
                          <span className="flex items-center gap-1 text-[10px] text-[var(--text-dim)]">
                            <Calendar size={9} />{formatDate(ad.startDate) || '∞'} — {formatDate(ad.endDate) || '∞'}
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        {adSubTab === 'archive' ? (
                          <button onClick={() => updateDoc(doc(db, 'ads', ad.id), { archived: false, active: true, endDate: null })}
                            className="flex items-center gap-1.5 px-3 py-2 bg-green-900/20 text-green-400 rounded-xl text-[9px] font-black uppercase hover:bg-green-700 hover:text-white">
                            <RotateCcw size={10} /> Przywróć
                          </button>
                        ) : (
                          <>
                            <button onClick={() => openEditAdForm(ad)} className="p-2.5 bg-[var(--bg-input)] text-[var(--text-dim)] rounded-xl hover:opacity-70"><Edit2 size={13} /></button>
                            <button onClick={() => updateDoc(doc(db, 'ads', ad.id), { active: !ad.active })}
                              className={`p-2.5 rounded-xl ${ad.active ? 'bg-green-900/20 text-green-400 hover:bg-green-700 hover:text-white' : 'bg-[var(--bg-input)] text-[var(--text-dim)] hover:opacity-70'}`}>
                              <Megaphone size={13} />
                            </button>
                            <button onClick={() => updateDoc(doc(db, 'ads', ad.id), { archived: true, active: false })} className="p-2.5 bg-[var(--bg-input)] text-[var(--text-dim)] rounded-xl hover:bg-orange-700 hover:text-white"><Archive size={13} /></button>
                            <button onClick={() => deleteDoc(doc(db, 'ads', ad.id))} className="p-2.5 bg-red-900/20 text-red-400 rounded-xl hover:bg-red-600 hover:text-white"><Trash2 size={13} /></button>
                          </>
                        )}
                      </div>
                    </div>
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[2.5rem] w-full max-w-lg max-h-[85vh] overflow-y-auto p-8 relative"
            ...
            <div className="flex items-start justify-between mb-5">
              <div className="flex-1 min-w-0 pr-4">
                ...
                <p className="text-[10px] font-black text-violet-500 uppercase tracking-widest">{previewRecipe.category}</p>
              </div>
              <button onClick={() => setPreviewRecipe(null)} className="p-2 bg-[var(--bg-input)] text-[var(--text-dim)] hover:text-[var(--text)] rounded-xl shrink-0">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="space-y-2 mb-5">
              {[
                { label: 'Spirytus 96%', value: previewRecipe.spirit_volume ? `${previewRecipe.spirit_volume} ml` : '—' },
                { label: 'Docelowa moc', value: previewRecipe.target_strength ? `${previewRecipe.target_strength}%` : '—' },
                { label: 'Faktyczna moc', value: previewRecipe.calc_final_strength ? `${Number(previewRecipe.calc_final_strength).toFixed(1)}%` : '—' },
                { label: 'Maceracja', value: previewRecipe.maceration_days ? `${previewRecipe.maceration_days} dni` : '—' },
              ].map(({ label, value }) => (
                <div key={label} className="flex justify-between p-3 bg-[var(--bg)] border border-[var(--border)] rounded-xl text-sm">
                  <span className="text-[var(--text-dim)] font-bold">{label}</span>
                  <span className="font-black text-[var(--text)]">{value}</span>
                </div>
              ))}
            </div>
            {previewRecipe.fruits?.length > 0 && (
              <div className="mb-5">
                <p className="text-[9px] font-black text-[var(--text-dim)] uppercase tracking-widest mb-2">Owoce</p>
                {previewRecipe.fruits.map((f, i) => (
                  <div key={i} className="flex justify-between p-2.5 bg-[var(--bg)] border border-[var(--border)] rounded-xl mb-1 text-sm">
                    <span className="font-bold text-[var(--text)]">{f.name}</span>
                    <span className="text-[var(--text-dim)] font-bold">{f.weight_g}g · {f.juice_yield_pct}%</span>
                  </div>
                ))}
              </div>
            )}
            <div className="pt-5 border-t border-[var(--border)]">
              {previewRecipe.blocked ? (
                <button onClick={() => handleBlockRecipe(previewRecipe.id, false)}
                  className="w-full py-3 bg-green-600 text-white rounded-2xl font-black uppercase text-xs flex items-center justify-center gap-2 hover:bg-green-700">
                  <Unlock size={14} /> Odblokuj recepturę
                </button>
              ) : (
                <button onClick={() => handleBlockRecipe(previewRecipe.id, true)}
                  className="w-full py-3 bg-red-600 text-white rounded-2xl font-black uppercase text-xs flex items-center justify-center gap-2 hover:bg-red-700">
