...
import { Home, BookOpen, Heart, User, FlaskConical } from 'lucide-react';

const TABS = [
  { id: 'home',       label: 'Home',      Icon: Home         },
  { id: 'recipes',    label: 'Receptury', Icon: BookOpen     },
  { id: 'my',         label: 'Moje',      Icon: Heart        },
  { id: 'calculator', label: 'Kalkulator',Icon: FlaskConical },
          ...
          return (
            <button
              ...
              className={`flex-1 flex flex-col items-center gap-0.5 py-2.5 transition-colors ${
                active ? 'text-violet-500' : 'text-[var(--text-dim)] hover:text-[var(--text)]'
              }`}
            >
              <Icon
                size={22}
              ...
              />
              <span className={`text-[9px] font-black uppercase tracking-wider ${active ? 'text-violet-500' : 'text-[var(--text-dim)]'}`}>
                {label}
              </span>
            </button>
          );
