...
const AuthModal = ({ onClose }) => {
  ...
  return (
    ...
    >
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[3rem] p-10 md:p-14 max-w-md w-full relative shadow-2xl border-t-[16px] border-t-red-600">
        ...
        <form onSubmit={handleEmailAuth} className="space-y-3 text-left text-[var(--text)]">
          <input type="email" required placeholder="E-mail" className="w-full border-2 border-[var(--border)] p-4 rounded-2xl font-bold bg-[var(--bg)] text-[var(--text)] placeholder-[var(--text-dim)] outline-none focus:border-red-600" value={email} onChange={e => setEmail(e.target.value)} />
          <div className="flex flex-col gap-2">
            <input type="password" required placeholder="Hasło" className="w-full border-2 border-[var(--border)] p-4 rounded-2xl font-bold bg-[var(--bg)] text-[var(--text)] placeholder-[var(--text-dim)] outline-none focus:border-red-600" value={password} onChange={e => setPassword(e.target.value)} />
            <button type="button" onClick={handleResetPassword} className="text-left text-[9px] font-bold text-red-500 uppercase px-4 leading-none">Zapomniałeś hasła?</button>
          </div>
          <button type="submit" className="w-full bg-[#DC2626] text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-2xl active:scale-95 transition-all mt-4">
            {authMode === 'login' ? 'Zaloguj się' : 'Załóż konto'}
          </button>
        </form>
        <button onClick={() => setAuthMode(authMode === 'login' ? 'register' : 'login')} className="mt-8 text-[10px] font-bold text-red-500 uppercase underline tracking-widest text-center">
