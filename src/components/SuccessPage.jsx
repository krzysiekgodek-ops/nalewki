  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center p-6">
      <div className="bg-[var(--bg-card)] rounded-[2rem] border border-[var(--border)] shadow-xl p-8 max-w-sm w-full text-center">
        ...
        {status === 'loading' && (
          <>
            <Loader2 size={40} className="mx-auto mb-4 text-violet-600 animate-spin" />
        ...
        {status === 'ok' && (
          <>
            ...
            <p className="text-sm text-[var(--text-dim)] font-medium mb-1">
              Plan <span className="font-black text-violet-500">{PLAN_LABELS[plan] || plan.toUpperCase()}</span> został aktywowany.
            </p>
            <p className="text-xs text-[var(--text-dim)] mb-8">Miłego korzystania z Nalewkarz Master!</p>
            <button onClick={goHome} className="w-full bg-violet-600 text-white py-3 rounded-2xl font-black uppercase text-sm hover:bg-violet-700 transition-all shadow-lg">
        ...
        {status === 'noauth' && (
          <>
            ...
            <button onClick={goHome} className="w-full bg-violet-600 text-white py-3 rounded-2xl font-black uppercase text-sm hover:bg-violet-700 transition-all">
              Zaloguj się
            </button>
          </>
        )}
