    ...
    >
      {ad.imageUrl && (
        <div className="relative w-full" style={{ height: '140px' }}>
          <img src={ad.imageUrl} alt={title} className="absolute inset-0 w-full h-full object-cover" />
      ...
      {activeAds.length > 1 && (
        <div className="absolute bottom-2 right-8 flex gap-1">
          {activeAds.map((_, i) => (
            <div key={i} className={`w-1.5 h-1.5 rounded-full transition-all ${i === index % activeAds.length ? 'bg-white' : 'bg-white/30'}`} />
          ))}
        </div>
      )}
    </div>
